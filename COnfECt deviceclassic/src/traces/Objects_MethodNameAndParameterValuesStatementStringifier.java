/*
 *  Objects_MethodNameAndParameterValuesStatementStringifier.java
 * 
...
/**
 * 
 * Statement stringifier which considers the name and parameter values of the method and the names and classes of the caller and the callee.  
 *
 *
 * @author Sylvain Lamprier
 *
 */
public class Objects_MethodNameAndParameterValuesStatementStringifier extends
		StatementsStringifier {

	/**
	 * Stringifies a statement according to the caller name, the calle name and the values of the method parameters.
	 * 
	 * The resulting string is in the form : Class(caller)[Name(caller)]:Class(callee)[Name(callee)].method(parameter values)
	 *   
	 * @param st
	...
	public Statement getStatement(String s){
		...
		for(String ss:st){
			...
			String[] sst=ss.split("\\[");
			ObjectClass c=new ObjectClass(sst[0]);
			//System.out.println(c);
			ObjectInstance p=new ObjectInstance(sst[1].substring(0, sst[1].length()-1)+"",c);
	...
	public static void main(String[] args){
		...
		Statement st7=new Statement("S1"+i++,atm0,new Method("blabla",new ArrayList<ObjectInstance>(),ihm),ihm0);
		 ...
		 Objects_MethodNameAndParameterValuesStatementStringifier o=new  Objects_MethodNameAndParameterValuesStatementStringifier();
