/*
 *  ObjectValues_MethodNameStatementStringifier.java
 * 
...
/**
 * 
 * Statement stringifier which only considers the name of the method and the values of variables and the classes  of the caller and the callee.
 *
 * @author Sylvain Lamprier
 *
 */
public class ObjectValues_MethodNameStatementStringifier extends StatementsStringifier {

	/**
	 * Stringifies a statement according to the instance variables of the caller and the calle of the corresponding method.
	 * 
	 * The resulting string is in the form : Class(caller)[Variables(caller)]:Class(callee)[Variables(callee)].method()
	 *   
	 * @param st
	...
	public String getText(Statement st) {
		...
		stb.append(callerclass.getName()+"[");
		ArrayList<ObjectInstance> vars=caller.getVars();
		...
		stb.append("]:");
		stb.append(calleeclass.getName()+"[");
		vars=callee.getVars();
		nvars=vars.size();
		for(int i=0;i<nvars;i++){
	...
	public static void main(String[] args){
		...
		ObjectValues_MethodNameStatementStringifier o=new  ObjectValues_MethodNameStatementStringifier();
